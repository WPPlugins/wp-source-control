...
if (!class_exists("SCTomM8")) {
  class SCTomM8 {
    ...
    function add_social_share_links($url) {
      ?>
      <a title="Share On Facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo($url); ?>"><img style="width: 30px;" src="<?php echo(get_option("siteurl")); ?>/wp-content/plugins/wp-source-control/images/facebook.jpg" style="width: 30px;" /></a>
      <a title="Share On Twitter" target="_blank" href="http://twitter.com/intent/tweet?url=<?php echo($url); ?>"><img style="width: 30px;" src="<?php echo(get_option("siteurl")); ?>/wp-content/plugins/wp-source-control/images/twitter.jpg" style="width: 30px;" /></a>
      <a title="Rate it 5 Star" target="_blank" href="<?php echo($url); ?>"><img style="padding-bottom: 3px;" src="<?php echo(get_option("siteurl")); ?>/wp-content/plugins/wp-source-control/images/rate-me.png" /></a>
    ...
    function get_query_string_value($name, $index = -1) {
      if ($index == -1) {
        if (isset($_POST[$name])) {
          return SCTomM8::fix_http_quotes($_POST[$name]);
        } else if (isset($_GET[$name])) {
          return SCTomM8::fix_http_quotes($_GET[$name]);
            ...
            $_POST[$name] = SCTomM8::fix_http_quotes($data);
            return SCTomM8::fix_http_quotes($data);
        } else {
          return "";
        }
      } else {
        ...
        if (isset($_POST[$name][$index])) {
          return SCTomM8::fix_http_quotes($_POST[$name][$index]);
        } else if (isset($_GET[$name][$index])) {
          return SCTomM8::fix_http_quotes($_GET[$name][$index]);
            ...
            $_POST[$name][$index] = SCTomM8::fix_http_quotes($data);
            return SCTomM8::fix_http_quotes($data);
        } else {
          return "";
        }
      ...
      if (is_array($uploadfiles)) {
        ...
        foreach ($uploadfiles['name'] as $key => $value) {
          ...
          if ($uploadfiles['error'][$key] == 0) {
            ...
            SCTomM8::update_record_by_id("posts", array("guid" => get_option("siteurl").$matches[0][0]), "ID", $attach_id);
            // echo $filedest;
          }
        }   
      }
    ...
    function generate_datatable($table_name, $fields_array, $primary_key_name, $where_clause, $order_array = array(), $limit_clause = "", $page_name, $display_show = true, $display_edit = true, $display_delete = true, $sortable_columns = false, $paginate_table = false, $date_format = "Y-m-d", $filter_arrays = array()) {
      ...
      if (!is_array($fields_array)) {
        ...
        if (count($filter_arrays) > 0) {
          if (SCTomM8::get_query_string_value($table_name."_filters") != "") {
            $filters = explode(",", SCTomM8::get_query_string_value($table_name."_filters"));
            foreach ($filters as $filter) {
              if (SCTomM8::get_query_string_value("filter_".$filter) != "") {
                if (!(isset($_POST["action"]) && $_POST["action"] == "Reset")) {
                  array_push($extra_where, $filter." LIKE '%".SCTomM8::get_query_string_value("filter_".$filter)."%'");  
                }
              }
            }
        ...
        $results = SCTomM8::get_results($table_name, $fields_array, $where_clause, $order_array, $limit_clause.$offset_clause);
        
        $total_count = count(SCTomM8::get_results($table_name, $fields_array, $where_clause));
              ...
              <fieldset>
                ...
                foreach($filter_arrays as $filter_array) { 
                  foreach($filter_array as $key => $value) { 
                    if (SCTomM8::get_query_string_value("filter_".$key) != "") {
                      if (isset($_POST["action"]) && $_POST["action"] == "Reset") {
                        $_POST["filter_".$key] = "";
                      }
                      $params_filter .= "&filter_".$key."=".SCTomM8::get_query_string_value("filter_".$key);
                    }
                    array_push($filters, $key);
                    SCTomM8::add_form_field(null, $value["type"], titlize_str($key), "filter_".$key, "filter_".$key, array(), "p", array(), $value["value_options"]);
                  }
                } 
                ?>
        ...
        if (count($results) > 0) { 
          if ($paginate_table) { SCTomM8::generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "top"); } ?>
            <table class="data">
              <thead>
                <tr>
                  <?php foreach($fields_array as $field_name) { ?>
                    <th class='<?php echo(strtolower(str_replace("_", "-", $field_name))); ?>'>
                      ...
                      <?php } ?>
                      <?php echo(SCTomM8::titlize_str($field_name)); ?>
                      <?php if ($sortable_columns) {
                  ...
                  <tr>
                    <?php foreach($fields_array as $field_name) { ?>
                      <td class='<?php echo(esc_html(strtolower(str_replace("_", "-", $field_name)))); ?>'>
                        <?php 
                          if (SCTomM8::is_valid_datetime($result->$field_name)) {
            ...
            <?php if ($paginate_table) { SCTomM8::generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "bottom"); } ?>
        <?php } else {
    ...
    function generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, $pagination_class) {
      ...
      $total_number_pages = intval($total_count / $limit_clause);
        $params_filter = "";
        $filters = explode(",", SCTomM8::get_query_string_value($table_name."_filters"));
        $params_filter .= "&".$table_name."_filters=".SCTomM8::get_query_string_value($table_name."_filters");
        foreach ($filters as $filter) {
          if (SCTomM8::get_query_string_value("filter_".$filter) != "") {
            $params_filter .= "&filter_".$filter."=".SCTomM8::get_query_string_value("filter_".$filter);
          }
        }
    ...
    function generate_displayview($table_name, $fields_array, $id_column_name, $id) {
      if (!is_array($fields_array)) {
        ...
        $result = SCTomM8::get_row_by_id($table_name, $fields_array, $id_column_name, $id);
        echo("<dl>");
    ...
    function validate_value($validation, $value, $error_session_name) {
      ...
      if ($value != "") {
        ...
        if (preg_match("/date | date|^date$/i", $validation)) {
          if (!SCTomM8::is_valid_datetime($value)) {
        ...
        if (preg_match("/email | email|^email$/i", $validation)) {
          if (!SCTomM8::is_valid_email($value)) {
        ...
        if (preg_match("/multi-emails | multi-emails|^multi-emails$/i", $validation)) {
          if (!SCTomM8::is_valid_emails($value)) {
    ...
    function validate_form($validations_array) {
      ...
      foreach ($validations_array as $key => $value) {
        if (is_array(SCTomM8::get_query_string_value($key))) {
          $index = 0;
          foreach (SCTomM8::get_query_string_value($key) as $sub_value) {
            if (SCTomM8::validate_value($value, $sub_value, $key."_".$index."_error") == false) {
              $validate_form = false;
            }
            $index++;
          }
          ...
          if (preg_match("/required/i", $value) && isset($_POST[$key."_0"])) {
            ...
            if (is_array(SCTomM8::get_query_string_value($key."_0"))) {
              // For checkbox fields.
              $index = 0;
              foreach ($_POST["validation_0"] as $row) {
                ...
                if (SCTomM8::validate_value($value, $data, $key."_".$index."_error") == false) {
                  echo $index;
                  $validate_form = false;
                }
                $index++;
              ...
              if (SCTomM8::validate_value($value, $data, $key."_error") == false) {
                $validate_form = false;
              }
            }
          } else {
            if (SCTomM8::validate_value($value, SCTomM8::get_query_string_value($key), $key."_error") == false) {
              $validate_form = false;
            }
          }
        }
    ...
    function add_form_field($instance, $field_type, $field_label, $field_id, $field_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      if ($instance == null && preg_match("/^tomm8te_admin_option::/", $field_name)) {
        ...
        if (count($_POST) > 0) {
          if ($field_index >= 0) {
            $field_value = SCTomM8::get_query_string_value($field_name, $field_index);
          } else {
            $field_value = SCTomM8::get_query_string_value($field_name);
          }
        }
      } else {
      ...
      if ($field_type == "text") {
        ...
        echo("<img id='$field_id_with_without_index' src='".get_option("siteurl")."/wp-content/plugins/wp-source-control/securimage/securimage_show.php' />");
        echo("<a href='#' onclick=\"document.getElementById('".$field_id_with_without_index."').src = '".get_option("siteurl")."/wp-content/plugins/wp-source-control/lib/securimage/securimage_show.php?' + Math.random(); return false\">[ Different Image ]</a><input type='text' name='".$field_name."' size='10' maxlength='6' />");
        ...
        if (count($value_options) == 1) {
          ...
          foreach($value_options as $key => $option) {
            ...
            $field_value = SCTomM8::get_query_string_value($field_name."_".$i, $field_index);
            $field_value = str_replace("&amp;", "&", htmlentities(htmlentities($field_value, ENT_NOQUOTES), ENT_QUOTES));
            $checked_value = "";
            if (count($_POST) == 0) {
    ...
    function add_option_form_field($field_type, $field_label, $field_id, $option_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      SCTomM8::add_form_field(null, $field_type, $field_label, $field_id, "tomm8te_admin_option::".$option_name, $field_attributes, $container_element, $container_attributes, $value_options, $field_index);
    }
